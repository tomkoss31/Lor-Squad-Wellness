# CLAUDE.md — archive du 18/09/2026

Sections retirées de `CLAUDE.md` le 18/09/2026 (lot 2 de l'audit : le fichier faisait
~1 500 lignes et était relu à chaque conversation). Elles sont conservées **telles quelles**
pour l'histoire des chantiers ; **rien ici ne fait foi sur l'état actuel du code** — les
fiches de chantier décrivent l'intention au cadrage, pas ce qui a été livré. Les règles encore
valables ont été condensées dans `CLAUDE.md`.
...
- **Fix messagerie `message_type` invisible (2026-06-08)** : l'edge `client-app-set-baseline` (onboarding mode `skip`) écrivait `message_type='message'`, valeur hors union → message client invisible dans la Messagerie (badge "+1" mais introuvable). Fix : edge → `'general'` ; backfill prod des 3 messages perdus (3 clients) ; `MessagesPage` catch-all (type inconnu → onglet « Demandes clients ») ; `InboxWidget` aligné sur `unreadMessageCount` ; **CHECK constraint** sur `client_messages.message_type` (échec écriture > perte silencieuse). Cf. règle « tout insert client_messages doit utiliser un type de l'union `domain.ts` ».
