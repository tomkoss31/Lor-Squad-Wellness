...
import { JournalIcone } from "../../journal/JournalIcone";
    ...
    <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 75, background: "rgba(0,0,0,.55)", display: "flex", alignItems: "flex-end" }}>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 11, padding: "16px 18px 13px", borderBottom: "1px solid var(--ls-bbc-line)", flex: "none" }}>
          {/* 21/09 (maquette v8) : Noaly prend le rose du site, rond comme son bouton de la barre. */}
          <div style={{ width: 42, height: 42, borderRadius: 999, background: "var(--ls-bbc-noaly-orbe)", boxShadow: "var(--ls-bbc-noaly-ombre)", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", flex: "none" }} aria-hidden="true">
            <JournalIcone nom="etincelle" taille={21} />
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 16, fontWeight: 700 }}>Noaly</div>
            <div style={{ fontSize: 11, color: "var(--ls-bbc-muted)" }}>ton assistante du club</div>
          </div>
          <button type="button" onClick={onClose} aria-label="Fermer" style={{ width: 44, height: 44, borderRadius: 12, background: "var(--ls-bbc-s2)", border: "1px solid var(--ls-bbc-line)", color: "var(--ls-bbc-muted)", cursor: "pointer", fontSize: 15 }}>✕</button>
        </div>
          ...
          {turns.length === 0 ? (
            <>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 4 }}>
                {SUGGESTIONS.map((s) => (
                  <button key={s} type="button" onClick={() => void ask(s)} style={{ minHeight: 44, padding: "0 14px", borderRadius: 999, background: "transparent", border: "1px solid var(--ls-bbc-noaly-line)", color: "var(--ls-bbc-text)", fontSize: 12.5, fontWeight: 500, cursor: "pointer", fontFamily: "var(--ls-bbc-font-body)" }}>
                    {s}
                  </button>
                ))}
              </div>
            ...
            turns.map((t, i) => (
              <div key={i} style={{ alignSelf: t.role === "user" ? "flex-end" : "flex-start", maxWidth: "88%", background: t.role === "user" ? "var(--ls-bbc-noaly-moi)" : "var(--ls-bbc-s2)", color: t.role === "user" ? "#fff" : "var(--ls-bbc-text)", border: t.role === "user" ? "none" : "1px solid var(--ls-bbc-line)", borderRadius: t.role === "user" ? "16px 16px 5px 16px" : "16px 16px 16px 5px", padding: "12px 14px", fontSize: 13.5, lineHeight: 1.5, whiteSpace: "pre-wrap" }}>
                {t.content}
              </div>
            ))
          )}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 9, padding: "12px 18px calc(18px + env(safe-area-inset-bottom))", borderTop: "1px solid var(--ls-bbc-line)", flex: "none" }}>
          ...
          />
          <button type="button" onClick={() => void ask(input)} disabled={busy || !input.trim()} style={{ width: 46, height: 46, flex: "none", border: 0, borderRadius: 13, background: busy || !input.trim() ? "var(--ls-bbc-s2)" : "var(--ls-bbc-noaly-btn)", cursor: busy || !input.trim() ? "default" : "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke={busy || !input.trim() ? "var(--ls-bbc-muted)" : "#fff"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M22 2 11 13M22 2l-7 20-4-9-9-4z" /></svg>
          </button>
        </div>
      </div>
    </div>
