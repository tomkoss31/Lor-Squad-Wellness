...
export function RecapModal({ clientName, recapToken, onClose }: RecapModalProps) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 300, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: '#13161C', border: '1px solid rgba(201,168,76,0.2)', borderRadius: 18, padding: 28, width: '100%', maxWidth: 400, maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 22 }}>
          ...
          <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 20, fontWeight: 800, color: '#F0EDE8', marginBottom: 4 }}>Bilan enregistré ✦</div>
          <div style={{ fontSize: 13, color: '#7A8099' }}>Le récap de {clientName} est prêt à partager.</div>
        </div>

        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 16 }}>
          <div style={{ background: '#F0EDE8', padding: 12, borderRadius: 12 }}>
            <QRCodeSVG value={recapUrl} size={140} fgColor="#0B0D11" bgColor="#F0EDE8" />
          </div>
        </div>

        <div onClick={copy} style={{ background: '#1A1E27', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14, cursor: 'pointer' }}>
          <div style={{ flex: 1, fontSize: 11, color: '#7A8099', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{recapUrl.replace('https://', '')}</div>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={() => { onClose(); navigate('/clients') }} style={{ flex: 1, padding: 12, borderRadius: 10, border: '1px solid rgba(255,255,255,0.1)', background: 'transparent', color: '#7A8099', fontFamily: 'DM Sans, sans-serif', fontSize: 13, cursor: 'pointer' }}>Retour clients</button>
          <button onClick={() => window.open(recapUrl, '_blank')} style={{ flex: 1, padding: 12, borderRadius: 10, border: 'none', background: '#C9A84C', color: '#0B0D11', fontFamily: 'Syne, sans-serif', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>Voir le récap</button>
        </div>
      </div>
    </div>
  )
