  ...
  return (
    <div style={{ background: 'var(--ls-surface)', border: '1px solid rgba(251,113,133,0.25)', borderRadius: 12, padding: 16, marginBottom: 4 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
        ...
        <button onClick={dismissAll} style={{ marginLeft: 'auto', fontSize: 10, color: 'var(--ls-text-hint)', background: 'rgba(255,255,255,0.04)', border: 'none', borderRadius: 6, padding: '3px 10px', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>× Ignorer tout</button>
      </div>
      {visible.map((client, i) => {
        ...
        return (
          <div key={client.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 0', borderTop: i > 0 ? '1px solid rgba(255,255,255,0.05)' : 'none' }}>
            ...
            <button onClick={() => dismissOne(client.id)} style={{ fontSize: 11, padding: '4px 10px', borderRadius: 7, background: 'rgba(255,255,255,0.04)', color: 'var(--ls-text-hint)', border: 'none', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', flexShrink: 0 }}>Ignorer</button>
          </div>
        )
      })}
    </div>
