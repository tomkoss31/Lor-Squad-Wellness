...
export function PvDismissAlert({ clients }: PvDismissAlertProps) {
  ...
  return (
    <div style={{ background: '#13161C', border: '1px solid rgba(251,113,133,0.25)', borderRadius: 12, padding: 16, marginBottom: 4 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
        ...
        <span style={{ fontSize: 10, color: '#4A5068', marginLeft: 2 }}>{visible.length} client{visible.length > 1 ? 's' : ''} dépassé{visible.length > 1 ? 's' : ''}</span>
        <button onClick={dismissAll} style={{ marginLeft: 'auto', fontSize: 10, color: '#4A5068', background: 'rgba(255,255,255,0.04)', border: 'none', borderRadius: 6, padding: '3px 10px', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>× Ignorer tout</button>
      </div>
      {visible.map((client, i) => {
        const initials = client.name.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase()
        return (
          <div key={client.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 0', borderTop: i > 0 ? '1px solid rgba(255,255,255,0.05)' : 'none' }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 13, fontWeight: 500, color: '#F0EDE8' }}>{client.name}</div>
            ...
            <button onClick={() => dismissOne(client.id)} style={{ fontSize: 11, padding: '4px 10px', borderRadius: 7, background: 'rgba(255,255,255,0.04)', color: '#4A5068', border: 'none', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', flexShrink: 0 }}>Ignorer</button>
          </div>
        )
      })}
    </div>
