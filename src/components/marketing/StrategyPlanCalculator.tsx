// =============================================================================
// StrategyPlanCalculator — le cœur du « Strategy Plan · Projection 12 mois ».
//
// Extrait de FormationCalculatorPage (2026-08-04) pour être RÉUTILISABLE : il
// vit maintenant à 2 endroits — la page /formation/calculateur (historique) ET
// l'onglet « Projection 12 mois » du Plan Marketing (pour le sortir de l'oubli,
// demande Thomas). Le composant ne rend QUE le calculateur (sliders → outputs) ;
// le titre/back sont portés par la page/onglet hôte.
//
// Logique calibrée France (1 VP = 1,50 € · marge 50 % · 1 RO = 1,50 €), formule
// 5-3-1. Disclaimer DGCCRF/Herbalife inclus (obligatoire).
// =============================================================================
...
const PRICE_VP = 1.5;
const RETAIL_MARGIN = 0.5;
const RO_VALUE = 1.5;
const SUP_AVG_VP = 3500;

const DEFAULTS = { newClients: 5, recurrentClients: 3, averageBasket: 75, newCoachs: 1 };
...
function compute(p: ComputeParams): MonthData[] {
  ...
  for (let m = 1; m <= 12; m++) {
    ...
    const ro = Math.round(n1 * SUP_AVG_VP * 0.05 + n2 * SUP_AVG_VP * 0.04 + n3 * SUP_AVG_VP * 0.02);
...
export function StrategyPlanCalculator() {
  ...
  const data = useMemo<MonthData[]>(() => {
    const raw = compute({ newC: newClients, recC: recurrentClients, panier: averageBasket, coachC: newCoachs });
    return raw.map((d) => ({
      ...d,
  ...
  const reachedRanks = useMemo<Set<HerbalifeRank>>(() => {
    ...
    const idxMax = Math.max(...data.map((d) => RANK_LADDER.indexOf(d.rank)).filter((i) => i >= 0), -1);
    if (idxMax >= 0) for (let i = 0; i <= idxMax; i++) reached.add(RANK_LADDER[i]);
    return reached;
  }, [data]);

  return (
    <div style={{ fontFamily: "DM Sans, sans-serif", color: "var(--ls-text)" }}>
      {/* CARD 1 : Paramètres */}
      <Card>
        <CardTitle>Tes paramètres mensuels</CardTitle>
        <SliderRow label="Nouveaux clients" value={newClients} onChange={setNewClients} min={2} max={15} step={1} />
        <SliderRow label="Récurrents qui repassent" value={recurrentClients} onChange={setRecurrentClients} min={0} max={10} step={1} />
        <SliderRow label="Panier moyen (VP)" value={averageBasket} onChange={setAverageBasket} min={50} max={150} step={5} />
        <SliderRow label="Nouveaux coachs" value={newCoachs} onChange={setNewCoachs} min={0} max={3} step={1} />
      </Card>

      {/* CARD 2 : Vision + outputs */}
      <Card>
        <CardTitle>Vision — et si seulement…</CardTitle>
        <div style={{ display: "flex", gap: 6, margin: "4px 0 22px", flexWrap: "wrap" }}>
          {VISION_MULTIPLIERS.map((mm) => {
            const active = visionMult === mm.value;
            return (
              <button
                key={mm.label}
                type="button"
                onClick={() => setVisionMult(mm.value)}
                style={{
                  flex: "1 1 80px",
                  padding: "9px 6px",
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", justifyContent: "center", margin: "14px 0 22px", padding: "14px 10px", background: "var(--ls-surface)", border: "0.5px solid var(--ls-border)", borderRadius: 14 }}>
          {RANK_LADDER.map((rank) => {
            const on = reachedRanks.has(rank);
            return (
              <div key={rank} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4, padding: "4px 6px", borderRadius: 10, opacity: on ? 1 : 0.32, filter: on ? "none" : "grayscale(0.6)", transition: "all 400ms ease", minWidth: 64 }}>
                <RankPinBadge rank={rank} size="sm" />
                <div style={{ fontSize: 8.5, letterSpacing: 0.4, textTransform: "uppercase", fontWeight: 700, color: on ? "var(--ls-text)" : "var(--ls-text-hint)", textAlign: "center", lineHeight: 1.15, maxWidth: 64 }}>
            ...
            <thead>
              <tr>
                {["Mois", "VP", "RO", "Rang"].map((h) => (
                  <th key={h} style={{ textAlign: "left", color: "var(--ls-text-muted)", fontWeight: 500, padding: "8px 10px", borderBottom: "0.5px solid var(--ls-border)", fontSize: 10, textTransform: "uppercase", letterSpacing: "0.07em" }}>{h}</th>
                ))}
                <th style={{ textAlign: "right", color: "var(--ls-text-muted)", fontWeight: 500, padding: "8px 10px", borderBottom: "0.5px solid var(--ls-border)", fontSize: 10, textTransform: "uppercase", letterSpacing: "0.07em" }}>Revenu*</th>
              </tr>
            </thead>
            <tbody>
              {data.map((d) => (
                <tr key={d.m}>
                  ...
                  <td style={{ padding: "9px 10px", borderBottom: "0.5px solid color-mix(in srgb, var(--ls-text) 4%, transparent)", color: "var(--ls-text)", fontWeight: 600, textAlign: "right" }}>{formatEur(d.revenue)}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
function Stat({ label, value, sub, tone }: { label: string; value: string; sub: string; tone: "gold" | "teal" }) {
  ...
  return (
    <div style={{ background: "var(--ls-surface2)", border: `0.5px solid color-mix(in srgb, ${color} 25%, var(--ls-border))`, borderRadius: 10, padding: 14 }}>
      ...
      <div style={{ fontSize: 22, fontFamily: "Syne, sans-serif", fontWeight: 800, color, letterSpacing: "-0.02em" }}>{value}</div>
      <div style={{ fontSize: 10, color: "var(--ls-text-hint)", marginTop: 4 }}>{sub}</div>
    </div>
  );
}
