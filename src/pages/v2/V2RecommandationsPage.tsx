...
export function V2RecommandationsPage() {
  ...
  useEffect(() => {
    supabase.from('bilans').select('*').order('created_at', { ascending: false }).limit(50)
      .then(({ data }) => { setBilans(data || []); setLoading(false) })
  }, [])

  const allRecs = bilans.flatMap(b => (b.recommendations || []).map((r: Recommendation) => ({ ...r, bilan: b })))
  const sorted = [...allRecs].sort((a, b) => ({ haute: 0, moyenne: 1, basse: 2 }[a.priority] - { haute: 0, moyenne: 1, basse: 2 }[b.priority]))
      ...
      {loading ? <p style={{ color: '#7A8099' }}>Chargement…</p> : sorted.length === 0
        ...
        : sorted.map((r, i) => (
          <div key={i} style={{ background: '#13161C', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, padding: 20, marginBottom: 12, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16 }}>
            ...
            <p style={{ color: '#4A5068', fontSize: 11, flexShrink: 0 }}>{new Date(r.bilan.created_at).toLocaleDateString('fr-FR')}</p>
          </div>
        ))}
    </div>
  )
