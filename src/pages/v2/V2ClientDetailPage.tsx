  ...
  return (
    <div style={{ padding: '32px 36px', fontFamily: 'DM Sans, sans-serif', maxWidth: 900 }}>
      ...
      <div style={{ display: 'flex', gap: 0, marginBottom: 24, borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        {tabs.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)} style={{ padding: '10px 18px', background: 'none', border: 'none', borderBottom: tab === t.key ? '2px solid #C9A84C' : '2px solid transparent', color: tab === t.key ? '#F0EDE8' : '#7A8099', fontSize: 13, fontWeight: tab === t.key ? 600 : 400, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', marginBottom: -1 }}>{t.label}</button>
        ))}
      </div>
      ...
      {tab === 'profil' && (
        <div style={{ background: '#13161C', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, padding: 28 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
            <div><label style={lbl}>Prénom</label><input style={inp} value={form.first_name ?? ''} onChange={e => setForm(f => ({ ...f, first_name: e.target.value }))} /></div>
            <div><label style={lbl}>Nom</label><input style={inp} value={form.last_name ?? ''} onChange={e => setForm(f => ({ ...f, last_name: e.target.value }))} /></div>
            <div><label style={lbl}>Email</label><input style={inp} value={form.email ?? ''} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} /></div>
            <div><label style={lbl}>Téléphone</label><input style={inp} value={form.phone ?? ''} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} /></div>
            <div>
              <label style={lbl}>Statut</label>
              <select style={inp} value={form.status ?? 'actif'} onChange={e => setForm(f => ({ ...f, status: e.target.value as Client['status'] }))}>
                <option value="actif">Actif</option><option value="pause">Pause</option><option value="inactif">Inactif</option>
              </select>
            </div>
            <div><label style={lbl}>Taille (cm)</label><input style={inp} type="number" value={form.height_cm ?? ''} onChange={e => setForm(f => ({ ...f, height_cm: Number(e.target.value) }))} /></div>
          </div>
          <div style={{ marginBottom: 16 }}><label style={lbl}>Objectif</label><textarea style={{ ...inp, minHeight: 80, resize: 'vertical' }} value={form.objective ?? ''} onChange={e => setForm(f => ({ ...f, objective: e.target.value }))} /></div>
          <div style={{ marginBottom: 20 }}><label style={lbl}>Notes coach</label><textarea style={{ ...inp, minHeight: 60, resize: 'vertical' }} value={form.notes ?? ''} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} /></div>
      ...
      {tab === 'bilans' && (
        <div>
          {bilans.length === 0 ? <LorEmptyState icon="📋" title="Aucun bilan" action={<LorButton onClick={() => navigate(`/v2/clients/${id}/bilan/new`)}>Nouveau bilan</LorButton>} /> : bilans.map(b => (
            <div key={b.id} style={{ background: '#13161C', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, padding: 20, marginBottom: 12 }}>
              <p style={{ color: '#F0EDE8', fontWeight: 600, fontSize: 14, marginBottom: 12 }}>{new Date(b.date).toLocaleDateString('fr-FR')}</p>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
                ...
                {b.water_liters != null && <LorScoreBar value={b.water_liters} max={3} color="#2DD4BF" label="Eau" unit="L" />}
              </div>
            </div>
          ))}
        </div>
      ...
      {tab === 'scan' && (
        <div>
          {scans.length === 0 ? <LorEmptyState icon="⚖️" title="Aucun body scan" action={<LorButton onClick={() => navigate(`/v2/clients/${id}/scan/new`)}>Nouveau scan</LorButton>} /> : (
            <>
              <div style={{ background: '#13161C', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, padding: 24, marginBottom: 16 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 16 }}>
                  ...
                  {scans[0].visceral_fat_level != null && <LorScoreBar value={scans[0].visceral_fat_level} max={59} color={(scans[0].visceral_fat_level ?? 0) >= 15 ? '#FB7185' : '#2DD4BF'} label="Graisse viscérale" />}
                </div>
              </div>
              {scans.length > 1 && (
                <div style={{ background: '#13161C', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, overflow: 'hidden' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                    <thead><tr style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>{['Date','Poids','MG%','MM kg','Eau%','VF'].map(h => <th key={h} style={{ padding: '12px 16px', textAlign: 'left', color: '#4A5068', fontSize: 11 }}>{h}</th>)}</tr></thead>
                    <tbody>{scans.slice(1).map(s => <tr key={s.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
      ...
      {tab === 'suivis' && (
        <div>
          ...
          {suivis.length === 0 ? <LorEmptyState icon="📅" title="Aucun suivi" /> : suivis.map(s => (
