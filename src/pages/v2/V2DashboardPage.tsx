...
interface Stats { activeClients: number; bilansThisMonth: number; renewalsSoon: number }
    ...
    async function load() {
      try {
        const [clientsRes, bilansRes, scansRes] = await Promise.all([
          ...
          supabase.from('bilans').select('id').gte('created_at', new Date(new Date().getFullYear(), new Date().getMonth(), 1).toISOString()),
        ...
        setStats({ activeClients: clients.filter(c => c.status === 'actif').length, bilansThisMonth: bilansRes.data?.length ?? 0, renewalsSoon: 0 })
      } finally { setLoading(false) }
    }
    load()
  }, [])

  const Skeleton = () => <div style={{ height: 80, background: '#13161C', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, animation: 'shimmer 1.5s ease-in-out infinite' }} />

  return (
    <div style={{ padding: '32px 36px', fontFamily: 'DM Sans, sans-serif', maxWidth: 1100 }}>
      <style>{`@keyframes shimmer{0%,100%{opacity:.4}50%{opacity:.7}} @keyframes spin{to{transform:rotate(360deg)}}`}</style>
      <div style={{ marginBottom: 32 }}>
        <p style={{ color: '#4A5068', fontSize: 12, marginBottom: 4 }}>Bonjour,</p>
        <h1 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 28, color: '#F0EDE8', letterSpacing: '-0.02em' }}>{profile?.full_name ?? 'Coach'} 👋</h1>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: 20 }}>
        <div style={{ background: '#13161C', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, padding: 24 }}>
          ...
          {loading ? [1,2,3].map(i => <Skeleton key={i} />) : recentClients.length === 0
            ? <p style={{ color: '#4A5068', fontSize: 13, textAlign: 'center', padding: '24px 0' }}>Aucun client</p>
            : recentClients.map(c => (
              <div key={c.id} onClick={() => navigate(`/v2/clients/${c.id}`)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 0', borderBottom: '1px solid rgba(255,255,255,0.05)', cursor: 'pointer' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ width: 34, height: 34, borderRadius: '50%', background: 'rgba(201,168,76,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#C9A84C', fontWeight: 700, fontSize: 13 }}>{c.first_name.charAt(0)}{c.last_name.charAt(0)}</div>
                  <div>
                    <p style={{ color: '#F0EDE8', fontSize: 13, fontWeight: 600 }}>{c.first_name} {c.last_name}</p>
                    <p style={{ color: '#4A5068', fontSize: 11 }}>{c.objective ?? '—'}</p>
                  </div>
                </div>
        ...
        <div style={{ background: '#13161C', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, padding: 24 }}>
          ...
          {loading ? [1,2,3].map(i => <Skeleton key={i} />) : recentScans.length === 0
            ? <p style={{ color: '#4A5068', fontSize: 13, textAlign: 'center', padding: '24px 0' }}>Aucun scan</p>
            : recentScans.map(s => (
      ...
      <button onClick={() => navigate('/v2/clients')} style={{ position: 'fixed', bottom: 32, right: 32, background: '#C9A84C', color: '#0B0D11', fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 13, padding: '14px 22px', borderRadius: 12, border: 'none', cursor: 'pointer', boxShadow: '0 8px 24px rgba(201,168,76,0.3)' }}>
        + Nouveau client
      </button>
    </div>
  )
