...
export function V2SuiviPVPage() {
  ...
  useEffect(() => {
    ...
    ]).then(([p, c]) => { setProduits(p.data || []); setClients(c.data || []); setLoading(false) })
  }, [])
  ...
  async function handleCreate() {
    ...
    const { data } = await supabase.from('client_produits').select('*').eq('status', 'actif').order('start_date', { ascending: false })
    setProduits(data || [])
  ...
  return (
    <div style={{ padding: '32px 36px', fontFamily: 'DM Sans, sans-serif', maxWidth: 1000 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
        <div>
          <p style={{ color: '#4A5068', fontSize: 12, marginBottom: 4 }}>Volume</p>
          <h1 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 26, color: '#F0EDE8' }}>Suivi PV</h1>
        </div>
        <LorButton onClick={() => setShowModal(true)}>+ Ajouter un programme</LorButton>
      </div>

      {/* Stats */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 28 }}>
        {[
          { label: 'PV total actifs', value: pvTotal.toFixed(2), color: '#C9A84C' },
          { label: 'Clients avec produits', value: new Set(produits.map(p => p.client_id)).size, color: '#2DD4BF' },
          { label: 'Renouvellements cette semaine', value: renewSoon, color: '#FB7185' },
        ].map(s => (
      ...
      {loading ? <p style={{ color: '#7A8099' }}>Chargement…</p> : produits.length === 0 ? (
        <LorEmptyState icon="◈" title="Aucun programme actif" subtitle="Ajoute le premier programme d'un client." action={<LorButton onClick={() => setShowModal(true)}>Ajouter</LorButton>} />
      ) : produits.map(p => {
          ...
          <div key={p.id} style={{ background: '#13161C', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, padding: 20, marginBottom: 12 }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                {daysLeft <= 7 ? <LorBadge variant="danger">Renouvellement imminent</LorBadge> : daysLeft <= 14 ? <LorBadge variant="warning">Renouvellement à prévoir</LorBadge> : <LorBadge variant="success">Actif</LorBadge>}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ background: '#13161C', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: 32, width: '100%', maxWidth: 480, fontFamily: 'DM Sans, sans-serif' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div><label style={labelStyle}>Nom du produit</label><input style={inputStyle} value={form.produit_name} onChange={e => setForm(f => ({ ...f, produit_name: e.target.value }))} placeholder="Formula 1…" /></div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                ...
                <div><label style={labelStyle}>Durée (jours)</label><input style={inputStyle} type="number" value={form.duration_days} onChange={e => setForm(f => ({ ...f, duration_days: Number(e.target.value) }))} /></div>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div><label style={labelStyle}>PV</label><input style={inputStyle} type="number" step="0.01" value={form.pv || ''} onChange={e => setForm(f => ({ ...f, pv: Number(e.target.value) }))} /></div>
                <div><label style={labelStyle}>Prix public</label><input style={inputStyle} type="number" step="0.01" value={form.price_public || ''} onChange={e => setForm(f => ({ ...f, price_public: Number(e.target.value) }))} /></div>
              </div>
