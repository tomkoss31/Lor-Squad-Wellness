      ...
      <div style={{ position: 'relative', zIndex: 1, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, maxWidth: 960, width: '100%' }}>
        ...
        <div style={{ background: '#13161C', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, padding: '48px 40px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', gap: 40 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '6px 14px', borderRadius: 20, border: '1px solid rgba(201,168,76,0.2)', background: 'rgba(201,168,76,0.08)', color: '#C9A84C', fontSize: 11, fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', width: 'fit-content' }}>
              Lor'Squad Wellness
            </span>
            <div>
              ...
              <p style={{ color: '#7A8099', fontSize: 14, lineHeight: 1.75 }}>
                Bilan guidé, body scan, suivi hebdomadaire et module PV — tout en un seul espace fluide.
              </p>
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {['Bilans guidés', 'Body Scan', 'Suivi client', 'Module PV'].map(tag => (
                <span key={tag} style={{ padding: '6px 14px', borderRadius: 20, border: '1px solid rgba(255,255,255,0.07)', background: 'rgba(255,255,255,0.03)', color: '#7A8099', fontSize: 12 }}>
                  {tag}
                </span>
              ))}
            </div>
          ...
          <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: 24 }}>
            <p style={{ color: '#4A5068', fontSize: 11 }}>© 2025 Lor'Squad Wellness — Espace coach</p>
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              ...
              <input
                ...
                style={{ background: '#1A1E27', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, padding: '11px 14px', color: '#F0EDE8', fontSize: 13, fontFamily: 'DM Sans, sans-serif', outline: 'none' }}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              ...
              <input
                ...
                onBlur={e => (e.target.style.borderColor = 'rgba(255,255,255,0.08)')}
              />
              <button type="button" onClick={() => setShowPassword(v => !v)} style={{ alignSelf: 'flex-start', background: 'none', border: 'none', color: '#4A5068', fontSize: 11, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>
