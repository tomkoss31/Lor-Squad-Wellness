...
export function RecapPage() {
  ...
  if (notFound) return (
    <div style={{ background: '#0B0D11', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, textAlign: 'center', fontFamily: 'DM Sans, sans-serif', color: '#F0EDE8' }}>
      <div>
        ...
        <div style={{ fontSize: 13, color: '#7A8099' }}>Ce lien est valable 90 jours après le bilan.</div>
      </div>
    </div>
  )
  ...
  const inp: React.CSSProperties = { flex: 1, background: '#1A1E27', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8, padding: '10px 12px', fontSize: 13, color: '#F0EDE8', fontFamily: 'DM Sans, sans-serif', outline: 'none' }
  ...
  return (
    <>
      ...
      <div style={{ background: '#0B0D11', minHeight: '100vh', color: '#F0EDE8', fontFamily: 'DM Sans, sans-serif', maxWidth: 480, margin: '0 auto' }}>
        ...
        <div style={{ background: 'linear-gradient(135deg,#13161C 0%,#1A1E27 100%)', padding: '28px 20px 24px', position: 'relative', overflow: 'hidden', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 18, position: 'relative', zIndex: 1 }}>
            ...
            <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 14, color: '#F0EDE8' }}>Lor&apos;<span style={{ color: '#C9A84C' }}>Squad</span> Wellness</div>
          </div>
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 22, fontWeight: 800, color: '#F0EDE8', marginBottom: 3 }}>{recap.client_first_name} {recap.client_last_name}</div>
            <div style={{ fontSize: 12, color: '#7A8099', marginBottom: 12 }}>Bilan du {fDate} · Coach {recap.coach_name}</div>
            {recap.program_title && (
        ...
        <div style={{ padding: '20px 20px 48px' }}>
          {/* Métriques */}
          <div style={{ fontSize: 10, color: '#4A5068', letterSpacing: '1.5px', textTransform: 'uppercase', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 12 }}>
            {[
              ...
              <div key={m.label} style={{ background: '#13161C', border: '1px solid rgba(255,255,255,0.07)', borderTop: `2px solid ${m.color}`, borderRadius: 10, padding: 12 }}>
                <div style={{ fontSize: 9, color: '#4A5068', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 6 }}>{m.label}</div>
                <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 18, fontWeight: 700, color: m.color, lineHeight: 1 }}>{m.val}</div>
                {m.sub && <div style={{ fontSize: 10, color: '#4A5068', marginTop: 3 }}>{m.sub}</div>}
              </div>
            ))}
          </div>
          ...
          {(scan.muscleMass || scan.hydration || scan.visceralFat) && (
            <div style={{ background: '#13161C', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 10, padding: 14, marginBottom: 20 }}>
              {[
                ...
                <div key={m.label} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 0', borderBottom: i < arr.length-1 ? '1px solid rgba(255,255,255,0.05)' : 'none' }}>
                  <div style={{ fontSize: 11, color: '#7A8099', width: 90, flexShrink: 0 }}>{m.label}</div>
          ...
          {recap.recommendations && recap.recommendations.length > 0 && (
            <>
              ...
              {recap.recommendations.map((r, i) => (
                <div key={i} style={{ background: '#13161C', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 10, padding: 12, marginBottom: 8, display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                  ...
                  <div><div style={{ fontSize: 13, fontWeight: 600, color: '#F0EDE8', marginBottom: 2 }}>{r.name}</div><div style={{ fontSize: 11, color: '#7A8099', lineHeight: 1.5 }}>{r.shortBenefit}</div></div>
                </div>
              ))}
          ...
          <div style={{ background: '#13161C', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, padding: 16, marginBottom: 20 }}>
            <div style={{ display: 'flex', gap: 14, alignItems: 'center', marginBottom: 14 }}>
              <div style={{ width: 82, height: 82, background: '#F0EDE8', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              ...
              <div style={{ minWidth: 0, flex: 1 }}>
                <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 13, fontWeight: 700, color: '#F0EDE8', marginBottom: 4 }}>Ton récap personnel</div>
                <div style={{ fontSize: 11, color: '#7A8099', lineHeight: 1.5, marginBottom: 8 }}>Scanne ou partage pour retrouver tes résultats.</div>
                <div onClick={copyLink} style={{ fontSize: 10, background: '#1A1E27', borderRadius: 6, padding: '5px 10px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
                  <span style={{ flex: 1, color: '#4A5068', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{recapUrl.replace('https://','')}</span>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              {[
                ...
                { l: 'SMS', bg: 'rgba(255,255,255,0.06)', c: '#7A8099', fn: shareSMS },
              ].map(b => (
          ...
          <div style={{ background: '#13161C', border: '1px solid rgba(201,168,76,0.15)', borderRadius: 12, padding: 16, marginBottom: 24 }}>
            <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 14, fontWeight: 700, color: '#F0EDE8', marginBottom: 4 }}>Partage ce moment bien-être</div>
            <div style={{ fontSize: 11, color: '#7A8099', marginBottom: 16, lineHeight: 1.6 }}>Note un prénom et un contact — ton coach s&apos;occupe du reste.</div>
            {sent ? (
              ...
              <>
                ...
                <button onClick={sendRefs} disabled={sending} style={{ width: '100%', background: sending ? 'rgba(201,168,76,0.4)' : '#C9A84C', color: '#0B0D11', border: 'none', borderRadius: 9, padding: 13, fontFamily: 'Syne, sans-serif', fontSize: 13, fontWeight: 700, cursor: sending ? 'not-allowed' : 'pointer', marginTop: 4 }}>
                  {sending ? 'Envoi...' : 'Envoyer mes recommandations ✦'}
                </button>
              </>
            )}
          </div>
          <div style={{ textAlign: 'center', fontSize: 10, color: '#4A5068' }}>Lor&apos;Squad Wellness · Récap confidentiel · Valable 90 jours</div>
        </div>
      </div>
    </>
  )
