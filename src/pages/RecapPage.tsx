...
export function RecapPage() {
  ...
  if (loading) return (
    <div style={{ background: 'var(--ls-bg)', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ width: 32, height: 32, border: '2px solid rgba(var(--ls-gold-rgb),0.2)', borderTop: '2px solid #C9A84C', borderRadius: '50%', animation: 'spin 0.7s linear infinite' }} />
    ...
    <>
      ...
      <div style={{ background: 'var(--ls-bg)', minHeight: '100vh', color: 'var(--ls-text)', fontFamily: 'DM Sans, sans-serif', maxWidth: 480, margin: '0 auto' }}>
        ...
        <div className="recap-hero" style={{ background: 'linear-gradient(135deg,var(--ls-surface) 0%,var(--ls-surface2) 100%)', padding: '28px 20px 24px', position: 'relative', overflow: 'hidden', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
          <div style={{ position: 'absolute', top: -40, right: -40, width: 160, height: 160, borderRadius: '50%', background: 'rgba(var(--ls-gold-rgb),0.08)', pointerEvents: 'none' }} />
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 18, position: 'relative', zIndex: 1 }}>
            <div style={{ width: 30, height: 30, background: 'var(--ls-gold)', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 14, color: 'var(--ls-text)' }}>Lor&apos;<span style={{ color: 'var(--ls-gold)' }}>Squad</span> Wellness</div>
          </div>

          <a href={GOOGLE_URL} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(var(--ls-gold-rgb),0.1)', border: '1px solid rgba(var(--ls-gold-rgb),0.25)', borderRadius: 20, padding: '8px 16px', marginBottom: 22, position: 'relative', zIndex: 1 }}>
            <span style={{ color: '#F0C96A', fontSize: 13, letterSpacing: 1 }}>★★★★★</span>
            <span style={{ fontSize: 12, color: 'var(--ls-gold)', fontWeight: 500 }}>Laisser un avis Google — La Base</span>
            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="var(--ls-gold)" strokeWidth="1.5" style={{ marginLeft: 'auto', flexShrink: 0 }}><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>
          </a>

          <div style={{ position: 'relative', zIndex: 1 }}>
            <div style={{ width: 52, height: 52, borderRadius: '50%', background: 'var(--ls-gold-bg)', color: 'var(--ls-gold)', border: '2px solid rgba(var(--ls-gold-rgb),0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Syne, sans-serif', fontSize: 18, fontWeight: 800, marginBottom: 10 }}>{initials}</div>
        ...
        <div style={{ padding: '20px 20px 48px' }}>
          ...
          <div style={{ fontSize: 10, color: 'var(--ls-text-hint)', letterSpacing: '1.5px', textTransform: 'uppercase', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 6 }}>
            <div style={{ width: 4, height: 4, borderRadius: '50%', background: 'var(--ls-gold)' }} />Ton bilan du jour
          </div>
          <div className="recap-metrics-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 12 }}>
            {[
              { label: 'Poids', val: scan.weight ? `${scan.weight} kg` : '—', color: 'var(--ls-gold)', sub: 'Point de départ' },
            ...
            <>
              ...
              {recap.recommendations.map((r, i) => (
                <div key={i} style={{ background: 'var(--ls-surface)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 10, padding: 12, marginBottom: 8, display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                  <div style={{ width: 30, height: 30, borderRadius: 8, background: i%2===0 ? 'rgba(45,212,191,0.1)' : 'rgba(var(--ls-gold-rgb),0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={i%2===0?'#2DD4BF':'var(--ls-gold)'} strokeWidth="1.5"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
                  </div>
            ...
            <div style={{ display: 'flex', gap: 14, alignItems: 'center', marginBottom: 14 }}>
              ...
              <div style={{ minWidth: 0, flex: 1 }}>
                ...
                <div role="button" tabIndex={0} onClick={copyLink} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); copyLink(); } }} style={{ fontSize: 10, background: 'var(--ls-surface2)', borderRadius: 6, padding: '5px 10px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <span style={{ color: copied ? '#2DD4BF' : 'var(--ls-gold)', fontWeight: 600, flexShrink: 0 }}>{copied ? '✓ Copié' : 'Copier'}</span>
                </div>
              </div>
            </div>
          ...
          <div style={{ fontSize: 10, color: 'var(--ls-text-hint)', letterSpacing: '1.5px', textTransform: 'uppercase', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 6 }}>
            <div style={{ width: 4, height: 4, borderRadius: '50%', background: 'var(--ls-gold)' }} />Tu connais quelqu&apos;un ?
          </div>
          <div style={{ background: 'var(--ls-surface)', border: '1px solid rgba(var(--ls-gold-rgb),0.15)', borderRadius: 12, padding: 16, marginBottom: 24 }}>
            ...
            {sent ? (
              ...
              <>
                ...
                <button onClick={sendRefs} disabled={sending} style={{ width: '100%', background: sending ? 'rgba(var(--ls-gold-rgb),0.4)' : 'var(--ls-gold)', color: 'var(--ls-bg)', border: 'none', borderRadius: 9, padding: 13, fontFamily: 'Syne, sans-serif', fontSize: 13, fontWeight: 700, cursor: sending ? 'not-allowed' : 'pointer', marginTop: 4 }}>
