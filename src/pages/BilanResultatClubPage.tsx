// =============================================================================
// BilanResultatClubPage — la page « Résultat Bilan » en habit Breakfast Club.
//
// Route : /resultat-club/:token   (public, prospect non authentifié)
//
// ── C'EST UNE COPIE, ET C'EST ASSUMÉ (Thomas, 09/09) ─────────────────────────
// « On garde la même page sur le modèle 360, mais avec une identité graphique
// BBC — duplique la page. »
//
// La règle B9 du projet dit « une feature, un seul endroit », et une copie de
// 1150 lignes la contredit. On l'a fait quand même, pour une raison précise :
// cette page porte un CHEMIN DE PAIEMENT VIVANT (`create-payment-link`,
// `confirm-stripe-payment`, le retour de caisse et son polling). Refactoriser
// l'original pour le rendre thémable, c'était toucher au seul écran qui
// encaisse. Dupliquer ne touche à rien de ce qui marche aujourd'hui.
//
// ⚠️ LE PRIX À PAYER, ET IL EST RÉEL : toute correction du paiement, des prix
// ou de l'analyse Noaly doit désormais être faite DEUX FOIS — ici et dans
// `BilanResultatPremiumPage.tsx`. Le jour où le paiement se stabilise, la
// bonne suite est d'extraire la logique dans un hook partagé et de ne garder
// en double que la présentation.
//
// ── CE QUI DIFFÈRE DE L'ORIGINAL, ET RIEN D'AUTRE ───────────────────────────
//   · le thème passe en `light` et la classe `.rbc` redéfinit les variables
//     de `PublicShell` avec les valeurs du club (cf. le .css à côté) ;
//   · Sora → Poppins pour le corps, et les deux styles de titre passent en
//     Anton capitales ;
...
import "./BilanResultatClubPage.css";
...
export function BilanResultatClubPage() {
  ...
  if (status === "loading") {
    return (
      <PublicShell defaultTheme="light" className="rbc">
  ...
  return (
    ...
    <PublicShell defaultTheme="light" showThemeToggle={false} className="rbc" style={{ maxWidth: "none", width: "100%" }}>
      <style>{RB_CSS}</style>
      <div style={{ fontFamily: "'Poppins', system-ui, sans-serif", color: "var(--cream)" }}>

        {/* ══ HEADER collant ══ */}
        <header style={{ position: "sticky", top: 0, zIndex: 60, background: "rgba(252,248,241,0.88)", backdropFilter: "blur(14px)", WebkitBackdropFilter: "blur(14px)", borderBottom: "1px solid var(--hair)" }}>
          <div style={{ maxWidth: 1180, margin: "0 auto", padding: "14px 24px", display: "flex", alignItems: "center", gap: 22 }}>
            <img src="/brand/breakfast-club/logo-heart.png" alt="The Breakfast Club by La Base" style={{ height: 26, width: "auto", display: "block" }} />
            <nav id="rb-nav" style={{ display: "flex", gap: 4, marginLeft: 8 }}>
              {CHAPTERS.map((c) => {
                const on = activeChapter === c.id;
                return (
                  <button key={c.id} type="button" onClick={() => scrollToId(c.id)} style={{ fontFamily: "'Poppins', sans-serif", fontSize: 13, fontWeight: on ? 700 : 500, color: on ? "var(--cream)" : "var(--cream-muted)", background: on ? "rgba(255,106,43,0.14)" : "transparent", border: `1px solid ${on ? "rgba(255,106,43,0.4)" : "transparent"}`, borderRadius: 999, padding: "6px 13px", cursor: "pointer", whiteSpace: "nowrap" }}>{c.label}</button>
                );
              })}
            </nav>
            {selectedProg && (
        ...
        <div id="rb-grid" style={{ maxWidth: 1180, margin: "0 auto", padding: "40px 24px 40px", display: "grid", gridTemplateColumns: "minmax(0, 1fr) 344px", gap: 44, alignItems: "start" }}>
          <main style={{ minWidth: 0, display: "flex", flexDirection: "column", gap: 64 }}>
            ...
            <section id="bilan" style={{ scrollMarginTop: 80, animation: "rb-fade .5s ease both" }}>
              ...
              <h1 style={{ fontFamily: "'Anton', sans-serif", fontWeight: 400, textTransform: "uppercase", fontSize: "clamp(30px, 4vw, 44px)", lineHeight: 1.1, letterSpacing: "-1px", margin: "0 0 16px" }}>
              ...
              <div id="rb-stats" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 24 }}>
                {stats.map((st) => (
                  <div key={st.label} style={inkCard}>
                    ...
                    <div style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 700, fontSize: 20, marginTop: 8, color: st.color }}>{st.value}</div>
                  </div>
                ))}
              </div>

              <div style={{ background: "linear-gradient(180deg, rgba(255,106,43,0.09), rgba(252,248,241,0.6))", border: "1px solid rgba(255,106,43,0.32)", borderRadius: 16, padding: 22 }}>
            ...
            <section id="formules" style={{ scrollMarginTop: 80 }}>
              ...
              <div style={{ marginTop: 22, background: "var(--ink2)", border: "1px solid var(--hair-strong)", borderRadius: 16, padding: 20 }}>
                <div style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 700, fontSize: 15.5, marginBottom: 4 }}>Une dernière chose, {firstName || "toi"} — où en es-tu&nbsp;?</div>
                ...
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                  {([
                    ...
                    return (
                      <button key={it.id} type="button" onClick={() => setIntent(it.id)} style={{ flex: "1 1 220px", textAlign: "left", cursor: "pointer", borderRadius: 13, padding: "13px 15px", background: on ? "linear-gradient(180deg, rgba(255,106,43,0.12), var(--ink))" : "var(--ink)", color: "var(--cream)", border: on ? "1.5px solid var(--teal)" : "1px solid var(--hair)" }}>
                        <div style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 700, fontSize: 14.5 }}>{it.title}</div>
                ...
                {programmes.map((p) => {
                  ...
                  return (
                    <button key={p.id} type="button" onClick={() => setSelected(p.id)} style={{
                      background: on ? "linear-gradient(180deg, rgba(255,106,43,0.12), rgba(252,248,241,0.9))" : "var(--ink2)",
                      border: on ? "1.5px solid var(--teal)" : reco ? "1px solid rgba(232,169,58,0.4)" : "1px solid var(--hair)",
              ...
              {addOn && (() => {
                ...
                return (
                  <div onMouseEnter={() => setSawAddon(true)} style={{ marginTop: 14, background: "linear-gradient(180deg, rgba(232,169,58,0.07), transparent)", border: "1px solid rgba(232,169,58,0.3)", borderRadius: 16, padding: 20, display: "flex", gap: 16, alignItems: "flex-start" }}>
                    <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 40, height: 40, borderRadius: 11, flexShrink: 0, background: "rgba(232,169,58,0.12)", border: "1px solid rgba(232,169,58,0.3)" }}>
                ...
                <div style={{ background: "linear-gradient(135deg, rgba(255,106,43,0.12), rgba(232,169,58,0.08))", border: "1px solid rgba(232,169,58,0.32)", borderRadius: 18, padding: 26, display: "flex", gap: 24, alignItems: "center", flexWrap: "wrap" }}>
                  ...
                  <div style={{ flex: 1, minWidth: 240, background: "var(--ink)", border: "1px solid var(--hair)", borderRadius: 14, padding: 18 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                      ...
                      <div style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 700, fontSize: 14.5 }}>L'appli We Do, dans ta poche</div>
                    </div>
                ...
                <div id="rb-why" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 12 }}>
                  {WHY.map((w) => (
                    <div key={w.title} style={{ background: "var(--ink2)", border: "1px solid var(--hair)", borderRadius: 14, padding: 18 }}>
                      <div style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 700, fontSize: 15 }}>{w.title}</div>
            ...
            <section id="demarrer" style={{ scrollMarginTop: 80 }}>
              <div style={{ textAlign: "center", background: "linear-gradient(180deg, rgba(255,106,43,0.1), rgba(252,248,241,0.6))", border: "1px solid var(--hair-strong)", borderRadius: 20, padding: "40px 28px" }}>
                ...
                {paidConfirmed ? (
                  <div style={{ maxWidth: 420, margin: "0 auto", textAlign: "left", background: "rgba(255,106,43,0.1)", border: "1px solid rgba(255,106,43,0.35)", borderRadius: 14, padding: 20 }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 14 }}>
                    <button type="button" disabled={payLoading} onClick={() => void startCheckout(selected)} style={{ ...gradBtn, width: "100%", maxWidth: 340, padding: "16px 24px", fontSize: 16, boxShadow: "0 8px 22px rgba(232,169,58,0.28)", opacity: payLoading ? 0.6 : 1, cursor: payLoading ? "wait" : "pointer" }}>{payLoading ? "Ouverture de la caisse…" : "Je démarre mon programme →"}</button>
            ...
            <div style={{ background: "var(--ink2)", border: "1px solid var(--hair-strong)", borderRadius: 18, padding: 22, boxShadow: "0 18px 50px -20px rgba(0,0,0,0.75)" }}>
              ...
              {paidConfirmed ? (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  <button type="button" disabled={payLoading} onClick={() => void startCheckout(selected)} style={{ ...gradBtn, width: "100%", padding: 15, borderRadius: 12, fontSize: 15, boxShadow: "0 8px 22px rgba(232,169,58,0.25)", opacity: payLoading ? 0.6 : 1, cursor: payLoading ? "wait" : "pointer" }}>{payLoading ? "Ouverture…" : "Je démarre →"}</button>
        ...
        {/* ══ ET APRÈS ══ */}
        <section style={{ borderTop: "1px solid var(--hair)", background: "rgba(252,248,241,0.4)" }}>
        ...
        <div id="rb-mobilebar" style={{ display: "none", position: "fixed", left: 0, right: 0, bottom: 0, zIndex: 70, background: "rgba(252,248,241,0.97)", backdropFilter: "blur(14px)", WebkitBackdropFilter: "blur(14px)", borderTop: "1px solid var(--hair-strong)", padding: "11px 16px 14px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, maxWidth: 640, margin: "0 auto" }}>
            ...
            {paidConfirmed ? (
              ...
              <button type="button" disabled={payLoading} onClick={() => void startCheckout(selected)} style={{ ...gradBtn, padding: "13px 22px", borderRadius: 12, fontSize: 14, whiteSpace: "nowrap", boxShadow: "0 6px 18px rgba(232,169,58,0.28)", opacity: payLoading ? 0.6 : 1 }}>{payLoading ? "…" : "Je démarre →"}</button>
            )}
          </div>
        </div>
        ...
        {showExit && (
          ...
          >
            <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 440, background: "var(--ink2)", border: "1px solid var(--hair-strong)", borderRadius: 20, padding: 26, boxShadow: "0 30px 80px rgba(0,0,0,0.7)" }}>
              ...
              {callbackState === "done" ? (
                <>
                  <div style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 800, fontSize: 19 }}>C'est noté, {firstName || "toi"} !</div>
                ...
                <>
                  <div style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 800, fontSize: 19 }}>Attends, {firstName || "toi"} — qu'est-ce qui te retient&nbsp;?</div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                    {[
                      ...
                      >
                        <div style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 700, fontSize: 14 }}>{o.k}</div>
...
const RB_CSS = `
/* Habit club : surface crème, et l'ambre à la place du lime (illisible ici). */
.public-shell{--ink2:#FFFFFF;--lime:#E8A93A}
