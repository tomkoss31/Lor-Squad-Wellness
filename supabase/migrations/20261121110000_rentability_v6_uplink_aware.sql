-- =============================================================================
-- Rentabilité V6 — uplink-aware (2026-05-21)
--
-- La RPC `get_users_rentability` et `get_user_rentability_history` utilisaient
-- jusqu'ici le rang du VIEWER comme distri_factor, ce qui surestimait la
-- marge si le client a un uplink HL différent (ex: Stéphanie suivie par
-- Mélanie 50% mais sous Ophélie 42%).
--
-- V6 : le distri_factor est calculé PAR LIGNE selon le rang du
-- `herbalife_uplink_user_id` du client (fallback `distributor_id` si null).
-- → Calcul rétroactif automatique pour tous les pv_client_products existants
--   dès que herbalife_uplink_user_id est rempli.
-- =============================================================================
...
begin
  ...
  -- Distri factor du user principal (utilisé en fallback si client n'a pas
  -- d'uplink HL différent — soit la grande majorité des cas).
  select
    u.current_rank::text,
  ...
  with current_products as (
    select pcp.id,
           ...
           -- V6 (2026-05-21) : distri_factor par ligne selon uplink HL réel
           -- (Ophélie 42% > Mélanie 50% pour Stéphanie). Fallback viewer.
           coalesce(
...
-- ============================================================================
-- Rebuild get_user_rentability_history avec même logique uplink-aware
-- ============================================================================
...
begin
  ...
  per_month as (
    select pp.m, sum(pp.price * pp.qty * greatest(0, pp.vip_factor - pp.effective_distri_factor))::numeric as eur
    from per_product pp
    group by pp.m
  )
  select
