-- =============================================================================
-- Rentabilité V3 — alignée sur la grille VIP officielle (2026-05-05)
--
-- Fix après précision Thomas : VIP ≠ 25 % fixe. Grille officielle
-- (cf. migration 20260428240000_client_vip_program.sql) :
--   - none       : 0%   (pas commandé)
--   - bronze     : 15%  (1ère commande)
--   - silver     : 25%  (≥100 PV lifetime)
--   - gold       : 35%  (≥500 PV lifetime)
--   - ambassador : 42%  (≥1000 PV 3 mois glissants)
--
-- Calcul marge nette distri par produit :
--   marge = prix × qty × greatest(0, margin_pct - vip_discount_pct) / 100
--
-- Exemples pour un Sup à 50 % :
--   - client none       → 50 % de marge
--   - client bronze     → 50 - 15 = 35 % de marge
--   - client silver     → 50 - 25 = 25 % de marge
--   - client gold       → 50 - 35 = 15 % de marge
--   - client ambassador → 50 - 42 =  8 % de marge
--
-- Pour un débutant à 25 % :
--   - client none       → 25 % de marge
--   - client bronze     → 25 - 15 = 10 %
--   - client silver+    → clamp à 0 % (pas de marge sur les VIP gros)
-- =============================================================================
...
returns table (
  ...
  margin_eur numeric,
  -- Split public / VIP (calcul correct par tier)
...
begin
  ...
  -- Sécurité : non-admin = caller in p_user_ids OR ancestor de tous les targets
  ...
  -- Pour chaque produit : on calcule sa marge nette distri en fonction
  -- du tier VIP DU CLIENT au moment de la requête (vip_status est cache).
  -- Grille officielle : none=0, bronze=15, silver=25, gold=35, ambassador=42.
  current_products as (
    select pcp.id,
           pcp.client_id,
           pcp.product_name,
  ...
  -- Aggregation par client pour le top 5
  by_client as (
    select
      cp.client_id,
      cp.client_name,
      ...
      -- Marge réelle de ce client : appliquer la formule correcte par tier
      sum(
  ...
  -- Split public vs VIP (avec calcul correct par tier sur chaque produit)
  agg_public as (
    select
  ...
  -- Mois précédent : marge totale (split pas nécessaire pour comparaison globale)
  agg_prev as (
    select
      coalesce(sum(
...
comment on function public.get_users_rentability is
  'Rentabilité V3 (2026-05-05) : grille VIP officielle (0/15/25/35/42 %) '
  'appliquée par tier au calcul de marge nette distri. Multi-user (couple) + '
  'split public/VIP + top 5 clients.';
