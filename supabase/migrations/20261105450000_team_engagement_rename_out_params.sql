-- =============================================================================
-- Fix definitif get_team_engagement : rename TOUS les OUT params (2026-05-05)
--
-- Les patches successifs (qualif role, qualif status, qualif user_id) finissent
-- en whack-a-mole. La VRAIE solution : prefixer tous les OUT params avec
-- `o_` pour qu'ils ne puissent JAMAIS clasher avec une colonne du body.
--
-- Le front (useTeamEngagement.ts) doit aussi etre updated pour mapper les
-- nouvelles colonnes. On garde la meme signature output mais via un wrapper
-- SELECT en sortie.
-- =============================================================================
...
as $$
  with recursive caller_check as (
    -- Auth check : caller doit etre admin ou ancetre de p_root_user_id
    select
  ...
  -- Sub-tree recursif depuis p_root_user_id
  sub_tree as (
  ...
  -- XP per user (formule alignee sur get_user_xp)
  xp_calc as (
    select
      st.st_id as u_id,
...
comment on function public.get_team_engagement is
  'RPC team engagement (V3 2026-05-05) : reecrite en SQL pur avec aliases '
  'CTE prefixes pour eviter les conflits avec OUT parameters de RETURNS TABLE. '
  'Plus de plpgsql declare/begin -> moins de surface ambiguite.';
